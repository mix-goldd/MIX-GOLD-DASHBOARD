...
function isRetryableResponse(httpStatus, payload) {
  return RETRYABLE_HTTP_STATUSES.has(Number(httpStatus)) || Number(payload?.status) === 429;
}
...
async function callApi(endpoint, params = {}, opts = {}) {
  return queueProviderRequest('vidmoly', async () => {
    ...
    const canFailover = !opts.keyId && opts.failover !== false;
    let lastError = null;

    for (let attempt = 0; attempt < (canFailover ? MAX_FAILOVER_ATTEMPTS : 1); attempt += 1) {
      const credential = opts.keyId
        ? await getApiKeyById('vidmoly', opts.keyId)
        : await getNextApiKey('vidmoly', { excludeIds: [...excludedIds] });
      excludedIds.add(credential.id);
      ...
      try {
        ...
        lastError = new Error(`Vidmoly API request failed to connect: ${err.message}`);
      ...
      try {
        ...
        lastError = new Error(`Vidmoly API returned non-JSON for ${endpoint}: ${bodyText.slice(0, 500)}`);
        if (!canFailover || !isRetryableResponse(res.status)) throw lastError;
        continue;
      }
      ...
      lastError = new Error(`Vidmoly API request failed (${res.status}): ${bodyText.slice(0, 500)}`);
      if (!canFailover || !isRetryableResponse(res.status, payload)) throw lastError;
    }

    throw lastError || new Error(`Vidmoly API request failed for ${endpoint}.`);
  });
}
...
const vidmoly = {
  ...
  getUploadServerForAccount: (keyId) => callApi('/upload/server', {}, { keyId }), // confirmed
...
module.exports._private = { API_BASE, MAX_FAILOVER_ATTEMPTS, RETRYABLE_HTTP_STATUSES, isRetryableResponse };
