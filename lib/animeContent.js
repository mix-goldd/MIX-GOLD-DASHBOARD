...
export const POST_TYPES = [
  ...
  { value: 'video', label: 'منشور / حلقة أنمي', icon: 'fa-play-circle' },
  { value: 'image', label: 'فن أنمي', icon: 'fa-palette' },
  { value: 'manga', label: 'فصل مانجا', icon: 'fa-book-open' },
  { value: 'model', label: 'شخصية أنمي', icon: 'fa-user-ninja' },
];
...
export function normalizeContentTypes(raw) {
  ...
  const unifiedVideoLabels = new Set(['منشور', 'حلقة أنمي', 'منشور حلقة أنمي', 'منشور / حلقة أنمي']);
  const normalized = source
    ...
    .map((entry) => {
      ...
      if (unifiedVideoValues.has(value) || unifiedVideoLabels.has(label)) {
        return { ...entry, value: 'video', label: 'منشور / حلقة أنمي', icon: entry.icon || 'fa-play-circle' };
      }
      return entry;
    })
    .filter((entry) => {
...
export const SIDEBAR_PARENTS = [
  ...
  { value: 'video', label: 'قائمة الأنمي' },
