...
// The portal's DATE report rounds every daily revenue row to cents before it
// shows the Total. Keep that report-compatible calculation separate from the
// raw API amount so the dashboard matches the exported Statistics CSV.
function statisticsRevenueFromPayload(payload) {
  return rowsFromPayload(payload).reduce((sum, row) => {
    return sum + roundCurrency(row.revenue ?? row.earnings ?? row.profit ?? row.amount);
  }, 0);
}
...
async function getCurrentMonthEarnings(now = new Date()) {
  ...
  return {
    total: statisticsRevenueFromPayload(payload),
    today: roundCurrency(byDay.get(today) || 0),
    yesterday: roundCurrency(byDay.get(yesterday) || 0),
...
async function getEarningsSummary(now = new Date()) {
  ...
  const monthlyTotals = historyPayloads.map((payload, index) => ({
    ...
    revenue: statisticsRevenueFromPayload(payload),
  }));
...
module.exports = { getCurrentMonthEarnings, getEarningsSummary, revenueFromPayload, statisticsRevenueFromPayload, rowsFromPayload };
