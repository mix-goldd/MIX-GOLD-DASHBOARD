...
// The Publisher Statistics API does not expose an account-balance field.
// Querying from a fixed early date gives the dashboard the full recorded
// revenue history while keeping it distinct from Adsterra's portal balance.
...
async function getEarningsSummary(now = new Date()) {
  ...
  const historyPayload = await fetchStats({ startDate: HISTORY_START, finishDate: today });

  return {
    ...currentMonth,
    historicalTotal: revenueFromPayload(historyPayload),
    historyPeriodStart: HISTORY_START,
