...
async function recordApiOutcome({ provider, keyId, httpStatus, retryAfter, providerPayload, now = Date.now() }) {
  ...
  if (limited) {
    // Vidmoly's observed daily limit has no dependable Retry-After header,
    // so the conservative fallback is the next UTC day rather than a
    // speculative retry loop.
    entry.blockedUntil = parseRetryAfter(retryAfter, now) || nextUtcMidnight(now);
