...
async function callGemini(body, opts = {}) {
  ...
  // A rate limit belongs to one credential, not necessarily to the account.
  // Record the response first so the quota manager pauses that key, then retry
  // the same request once per remaining configured credential.
  while (true) {
    let credential;
    try {
      credential = await getNextApiKey('gemini', { excludeIds: [...attemptedKeyIds] });
    } catch (err) {
      if (attemptedKeyIds.length) {
    ...
    attemptedKeyIds.push(credential.id);
