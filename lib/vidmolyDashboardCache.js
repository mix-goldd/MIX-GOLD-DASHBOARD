...
const SNAPSHOT_KEYS = {
  ...
  folders: 'vidmoly_folders_snapshot_v1',
};

// 120 minutes: with a 50/day Vidmoly quota shared across every page in the
// dashboard, a snapshot that can refresh every 30 minutes still adds up to
// ~48 requests/day from normal page visits alone, before any real upload
// work happens. A 2-hour window caps each snapshot kind at ~12 refreshes/day.
const TTL_MS = {
  library: 120 * 60 * 1000,
  earnings: 120 * 60 * 1000,
  // Folder trees are only used to populate picker dropdowns, so a slightly
  // stale list is harmless — this is what stops every visit to the upload
  // page from spending a live request just to draw the folder dropdown.
  folders: 120 * 60 * 1000,
};
...
function createVidmolyDashboardCache({
  ...
  async function getOrRefresh(kind, loader, { force = false, shouldPersist = () => true } = {}) {
    if (!isKnownKind(kind)) throw new Error(`Unknown Vidmoly snapshot kind: ${kind}`);
    ...
    const refresh = (async () => {
      try {
        const payload = await loader(existing?.payload || null);
        const persist = await shouldPersist(payload, existing?.payload || null);
        if (!persist && existing) {
          // A quota-limited library response can be syntactically successful
          // while containing only a fraction of the accounts (or none). Never
          // replace the durable last-known file and size snapshot with it.
  ...
  async function markStale(kind) {
    if (!isKnownKind(kind)) throw new Error(`Unknown Vidmoly snapshot kind: ${kind}`);
    inFlightRefreshes.delete(kind);
    const existing = await readSnapshot(kind);
    if (!existing) return null;

    // Expire the snapshot immediately without deleting its durable payload.
    // The next provider response can refresh it, while a quota wait still
    // leaves all already-known files and their stored sizes visible.
    const cachedAt = Number(now()) - TTL_MS[kind];
    await saveSetting(SNAPSHOT_KEYS[kind], { cachedAt, payload: existing.payload });
    return existing.payload;
  }

  return { getOrRefresh, invalidate, markStale, patchLibraryFile };
}
...
module.exports = {
  ...
  markVidmolySnapshotStale: defaultCache.markStale,
